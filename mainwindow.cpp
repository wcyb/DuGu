...
void MainWindow::selectFile()
{
    ...
    if (file.length())
    {
        if (!ui->tbOptions->item(sender()->objectName().toInt(), 1)) //if at given position widget not exist, create new
        {
            ...
            ui->tbOptions->item(sender()->objectName().toInt(), 1)
                ->setText(file); //if widget exist then set new text with path to file
    }
}
...
void MainWindow::startOperation()
{
    ...
    arguments.append("-p");
    arguments.append(settings.value(sParts, QMap<QString, QVariant>())
                         .toMap()
                         .key(ui->cbPart->currentText())); //get argument based on selected description
    arguments.append("-c");
    arguments.append(
        settings.value(sProgrammers, QMap<QString, QVariant>()).toMap().key(ui->cbProgrammer->currentText()));
    settings.endGroup();
    ...
    if (pathToAvrDude.isEmpty())
    {
        QMessageBox::critical(this,
                              tr("Error"),
                              tr("Path to AVRDUDE has not been set.\nTo set path, use options menu."));
        return;
    }
    ...
    //if memory type is not other (to write fusebits or use other additional options), we need file to write or read to
    if ((ui->cbMemoryType->currentIndex() != 2 || operationType == "r") && pathToFile.isEmpty())
    {
        QMessageBox::critical(this,
                              tr("Error"),
                              tr("Path to file has not been set.\nSet path to execute selected operation."));
        return;
    }

    //if file will not have .bin .hex. or .srec extension then use 'a' option so AVRDUDE will try to recognize filetype by itself
    QString fileType = getFileType(pathToFile);

    if (fileType == "a" && operationType == "r") //check if correct filetype has been selected for data readout
    {
        QMessageBox::critical(
            this,
            tr("Error"),
            tr("Wrong filetype has been selected.\nFor data readout, only HEX, BIN and SREC filetypes can be used."));
        return;
    }

    if (operationType == "w" || operationType == "v") //write or verify
    {
        if (ui->chbLowFuse->isChecked() && ui->leLowFuse->text().length())
        {
            arguments.append("-U");
            arguments.append("lfuse:" + operationType + ":0x" + ui->leLowFuse->text() + ":m");
        } else if (ui->chbLowFuse->isChecked() && !ui->leLowFuse->text().length())
        {
            QMessageBox::critical(
                this,
                tr("Error"),
                tr("Value of Low fusebit has not been set.\nSet this value to execute selected operation."));
            return;
        }

        if (ui->chbHighFuse->isChecked() && ui->leHighFuse->text().length())
        {
            arguments.append("-U");
            arguments.append("hfuse:" + operationType + ":0x" + ui->leHighFuse->text() + ":m");
        } else if (ui->chbHighFuse->isChecked() && !ui->leHighFuse->text().length())
        {
            QMessageBox::critical(
                this,
                tr("Error"),
                tr("Value of High fusebit has not been set.\nSet this value to execute selected operation."));
            return;
        }

        if (ui->chbExtendedFuse->isChecked() && ui->leExtendedFuse->text().length())
        {
            arguments.append("-U");
            arguments.append("efuse:" + operationType + ":0x" + ui->leExtendedFuse->text() + ":m");
        } else if (ui->chbExtendedFuse->isChecked() && !ui->leExtendedFuse->text().length())
        {
            QMessageBox::critical(
                this,
                tr("Error"),
                tr("Value of Extended fusebit has not been set.\nSet this value to execute selected operation."));
            return;
        }

        if (ui->chbSingleFuse->isChecked() && ui->leSingleFuse->text().length())
        {
            arguments.append("-U");
            arguments.append("fuse:" + operationType + ":0x" + ui->leSingleFuse->text() + ":m");
        } else if (ui->chbSingleFuse->isChecked() && !ui->leSingleFuse->text().length())
        {
            QMessageBox::critical(
                this,
                tr("Error"),
                tr("Value of Single fusebit has not been set.\nSet this value to execute selected operation."));
            return;
        }
    } else //read
    {
        ...
        if (ui->chbLowFuse->isChecked())
        {
            arguments.append("-U");
            arguments.append("lfuse:" + operationType + ":" + f.canonicalPath() + QDir::separator() + f.baseName()
                             + "_lfuse." + f.completeSuffix() + ":h");
        }
        if (ui->chbHighFuse->isChecked())
        {
            arguments.append("-U");
            arguments.append("hfuse:" + operationType + ":" + f.canonicalPath() + QDir::separator() + f.baseName()
                             + "_hfuse." + f.completeSuffix() + ":h");
        }
        if (ui->chbExtendedFuse->isChecked())
        {
            arguments.append("-U");
            arguments.append("efuse:" + operationType + ":" + f.canonicalPath() + QDir::separator() + f.baseName()
                             + "_efuse." + f.completeSuffix() + ":h");
        }
        if (ui->chbSingleFuse->isChecked())
        {
            arguments.append("-U");
            arguments.append("fuse:" + operationType + ":" + f.canonicalPath() + QDir::separator() + f.baseName()
                             + "_fuse." + f.completeSuffix() + ":h");
        }
    }

    if (ui->leProgrammerPort->text().length())
    {
        arguments.append("-P");
        arguments.append(ui->leProgrammerPort->text());
    }
    if (ui->cbMemoryType->currentIndex() == 0)
    {
        arguments.append("-U");
        arguments.append("flash:" + operationType + ":" + pathToFile + ":" + fileType);
    } else if (ui->cbMemoryType->currentIndex() == 1)
    {
        arguments.append("-U");
        arguments.append("eeprom:" + operationType + ":" + pathToFile + ":" + fileType);
    }

    //check all additional arguments if default value changed, if so then add changed parameter to AVRDUDE
    for (unsigned short i = 0; i < ui->tbOptions->rowCount(); i++)
    {
        switch (getOptionType(i))
        {
                //text and file path are treated the same, function getOptionWithArg will determine correct argument call
            case additionalOptionsTypes::text:
            case additionalOptionsTypes::filePath:
                if (!ui->tbOptions->model()->index(i, 1).data().toString().isEmpty())
                    arguments.append(getOptionWithArg(i,
                                                      ui->tbOptions->model()->index(i, 1).data().toString(),
                                                      operationType,
                                                      fileType));
                break;
    ...
    arguments.append("-s"); //disable safemode prompting
    ...
    connect(m_ad,
            ...
            [=](/*int exitCode, QProcess::ExitStatus exitStatus*/) {
                ui->statusBar->showMessage(tr("Ready"));
                ui->bStart->setEnabled(true);
                updateFusebytesValues(ui->pteOutput->toPlainText());
            });
    ui->pteOutput->clear();
...
void MainWindow::updateOutputInformation()
{
    ...
    if (matchesPr.hasNext())
    {
        ...
        ui->progressBar->setValue(ui->progressBar->value()
                                  + (prc.length() * 2)); //one '#' means 2% progress, max is 50 '#' chars on output
    }
}

void MainWindow::updateFusePlaceholderText(QRegularExpressionMatchIterator* regexIterator, QWidget* fuseByteLineEdit)
{
    if (regexIterator->hasNext())
    {
        QStringList val = regexIterator->next().captured().split(":");
        if (val.length() == 2) dynamic_cast<QLineEdit*>(fuseByteLineEdit)->setPlaceholderText(val.last());
    }
}

void MainWindow::updateFusebytesValues(QString adOutput)
{
    QRegularExpression reExtendedFuse("E\\:[0-9A-F]+");
    QRegularExpression reHighFuse("H\\:[0-9A-F]+");
    QRegularExpression reLowFuse("L\\:[0-9A-F]+");
    QRegularExpression reSingleFuse("S\\:[0-9A-F]+");

    QRegularExpressionMatchIterator matchExtFuse = reExtendedFuse.globalMatch(adOutput);
    QRegularExpressionMatchIterator matchHighFuse = reHighFuse.globalMatch(adOutput);
    QRegularExpressionMatchIterator matchLowFuse = reLowFuse.globalMatch(adOutput);
    QRegularExpressionMatchIterator matchSingleFuse = reSingleFuse.globalMatch(adOutput);

    updateFusePlaceholderText(&matchExtFuse, ui->leExtendedFuse);
    updateFusePlaceholderText(&matchHighFuse, ui->leHighFuse);
    updateFusePlaceholderText(&matchLowFuse, ui->leLowFuse);
    updateFusePlaceholderText(&matchSingleFuse, ui->leSingleFuse);
}
    ...
    for (unsigned short i = 0; i < getOptionsCount(); i++)
    {
        ...
        if (p.first.isEmpty())
            ui->tbOptions
                ->setCellWidget(i,
                                0,
                                p.second); //if description of widget is empty (ex. button) then set it in first column
        else
        {
            ...
            ui->tbOptions->setCellWidget(
                i, 1, p.second); //and second widget, generated in getOptionWidget (ex. combobox) in second column
        }
    }
...
void MainWindow::addTargetToMenu(QString targetName, QString nameInMenu)
{
    ...
    connect(act,
            ...
            this,
            SLOT(changeTarget(bool))); //use SIGNAL and SLOT because function is overloaded
...
void MainWindow::readSettings(bool updateCurrentData)
{
    ...
    if (!settings.allKeys().size())
    {
        QMessageBox::information(
            this,
            tr("Information"),
            tr("You need to initialize informations about parts and programmers.\nTo do this, use options menu."));
        return;
    }
    ...
    foreach (QVariant value, settings.value(sParts, QMap<QString, QVariant>()).toMap().values())
        ui->cbPart->addItem(value.toString());
    foreach (QVariant value, settings.value(sProgrammers, QMap<QString, QVariant>()).toMap().values())
    {
        ...
        ui->cbProgrammer->setItemData(ui->cbProgrammer->count() - 1,
                                      ui->cbProgrammer->itemText(ui->cbProgrammer->count() - 1),
                                      Qt::ToolTipRole);
    }
    ...
    if (!ui->cbPart->count() || !ui->cbProgrammer->count())
    {
        QMessageBox::information(this,
                                 tr("Information"),
                                 tr("To start using this program, you need to\ninitialize informations about parts and "
                                    "programmers.\nTo do this, use options menu."));
        return;
    }
    ...
    foreach (QString tg, settings.childGroups())
    {
        ...
        if (settings.value(sTargetDefault, bool(false)).toBool()
            && updateCurrentData) //if some target has been set as default, then read data from that target and update interface if it was allowed by arg
        {
            changeTarget(tg);
...
QString MainWindow::getFileType(QString pathToFile) const
{
    QStringList data = QFileInfo(pathToFile).fileName().split('.');

    if (data.last() == "hex")
        return "i";
    else if (data.last() == "srec")
        return "s";
    else if (data.last() == "bin")
        return "r"; //treat file with .bin extension as raw binary

    return "a"; //in every other case return type as 'a' for auto identification by AVRDUDE
}
...
QString MainWindow::getFile()
{
    ...
    if (ui->cbOperationType->currentIndex() != 1)
        file = QFileDialog::getOpenFileName(this,
                                            tr("Select data file"),
                                            QString(),
                                            "Intel HEX (*.hex);;Raw binary (*.bin);;Motorola S-record (*.srec);;ELF "
                                            "(*.elf);;Custom (*.*)"); //for write or verify
    else
    {
        file = QFileDialog::getSaveFileName(
            ...
            tr("Select data file"),
            QString(),
            "Intel HEX (*.hex);;Raw binary (*.bin);;Motorola S-record (*.srec);;Custom (*.*)");
        if (!QFileInfo(file).exists())
            if (!QFile(file).open(QIODevice::OpenModeFlag::ReadWrite))
                QMessageBox::critical(this,
                                      tr("Error"),
                                      tr("Selected file cannot be created.\nCheck if you have required premissions."));
    }
...
QPair<QString, QWidget*> MainWindow::getOptionWidget(unsigned short optionNumber)
{
    ...
    switch (m_additionalOptionsValues
                [optionNumber]) //create new widget depending on selected option, objectName will be set to optionNumber
    {
        case additionalOptionsTypes::logic:
        {
...
QStringList MainWindow::getOptionWithArg(unsigned short optionNumber,
                                         QString argument,
                                         QString operationType,
                                         QString valueType) const
{
    ...
    if (m_additionalOptionsArg[optionNumber].length() > 2)
        return QStringList() << "-U"
                             << m_additionalOptionsArg[optionNumber] + ":" + operationType + ":" + argument + ":"
                                    + valueType;
...
{
    QString aboutCaption = QMessageBox::tr("<h2>About DuGu</h2>"
    ...
    QString aboutText = QMessageBox::tr(
                            ...
                            "<p></p>")
                            .arg(QStringLiteral("https://fontawesome.com/license"),
                                 QCoreApplication::applicationVersion(),
                                 QStringLiteral("https://github.com/wcyb"));
    QMessageBox msg(this);
    msg.setWindowTitle(tr("About DuGu"));
    msg.setText(aboutCaption);
    msg.setInformativeText(aboutText);
    msg.setIconPixmap(
        QPixmap(":/images/microchip-solid.png")
            .scaled(256, 256, Qt::AspectRatioMode::KeepAspectRatio, Qt::TransformationMode::SmoothTransformation));
    msg.exec();
}
...
void MainWindow::addTarget()
{
    ...
    for (unsigned short freeGroupId = 0; freeGroupId < settings.childGroups().size(); freeGroupId++)
    {
        ...
        if (settings.childGroups().indexOf(newGroupName) != -1)
            freeGroupId++;
        else
        {
    ...
    {
        newGroupName = QString(sTargetData) + "_"
                       + QString::number(settings.childGroups().last().split('_').last().toShort() + 1);
        settings.beginGroup(newGroupName);
    ...
    if (ui->leProgrammerPort->text().length())
        settings.setValue(sTargetProgrammerPortName, ui->leProgrammerPort->text());
    ...
    for (unsigned short i = 0; i < ui->tbOptions->rowCount(); i++)
    {
        switch (getOptionType(i))
        {
            ...
            case additionalOptionsTypes::filePath:
                if (!ui->tbOptions->model()->index(i, 1).data().toString().isEmpty())
                    settings.setValue(QString::number(i), ui->tbOptions->model()->index(i, 1).data().toString());
                break;
...
void MainWindow::changeTarget(QString newTarget)
{
    ...
    if (!settings.value(sTargetLowFuse, QString()).toString().isEmpty())
        ui->leLowFuse->setText(settings.value(sTargetLowFuse, QString()).toString());
    if (!settings.value(sTargetHighFuse, QString()).toString().isEmpty())
        ui->leHighFuse->setText(settings.value(sTargetHighFuse, QString()).toString());
    if (!settings.value(sTargetExtendedFuse, QString()).toString().isEmpty())
        ui->leExtendedFuse->setText(settings.value(sTargetExtendedFuse, QString()).toString());
    if (!settings.value(sTargetSingleFuse, QString()).toString().isEmpty())
        ui->leSingleFuse->setText(settings.value(sTargetSingleFuse, QString()).toString());

    ui->cbProgrammer->setCurrentIndex(settings.value(sTargetProgrammerId, int()).toInt());

    if (!settings.value(sTargetProgrammerPortName, QString()).toString().isEmpty())
        ui->leProgrammerPort->setText(settings.value(sTargetProgrammerPortName, QString()).toString());
    if (!settings.value(sTargetFile, QString()).toString().isEmpty())
        ui->leFilePath->setText(settings.value(sTargetFile, QString()).toString());
    ...
    for (unsigned short i = 0; i < ui->tbOptions->rowCount(); i++)
    {
        if (settings.value(QString::number(i), QString()).toString().isEmpty())
            continue; //if some value was not changed, then check next
        ...
        switch (getOptionType(i))
        {
            ...
            case additionalOptionsTypes::logic:
                ui->tbOptions->cellWidget(i, 1)
                    ->findChild<QCheckBox*>(QString::number(i))
                    ->setCheckState(Qt::CheckState::Checked);
                break;

            case additionalOptionsTypes::itemList:
                ui->tbOptions->cellWidget(i, 1)
                    ->findChild<QComboBox*>(QString::number(i))
                    ->setCurrentIndex(settings.value(QString::number(i), int()).toInt());
                break;
        }
    }
